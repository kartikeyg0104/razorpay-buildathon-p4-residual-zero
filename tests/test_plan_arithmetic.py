...
def test_plan_p2_ladder_estimates_sum_to_71():
    """PLAN-P2.md checkpoint headings must sum to the spec's 71h."""
    plan = Path("PLAN-P2.md")
    if not plan.exists():
        pytest.skip("PLAN-P2.md not present")
    hours = [
        int(h)
        for h in re.findall(
            r"^### CP2\.\d+ · .+? · (\d+)h · trip-wire",
            plan.read_text(encoding="utf-8"),
            re.M,
        )
    ]
    assert len(hours) == 12, f"expected 12 Phase 2 checkpoints, found {len(hours)}"
    assert sum(hours) == 71, f"P2 ladder sums to {sum(hours)}h, not 71h"


def test_plan_p3_ladder_estimates_sum_to_69():
    """PLAN-P3.md checkpoint headings must sum to the spec's 69h."""
    plan = Path("PLAN-P3.md")
    if not plan.exists():
        pytest.skip("PLAN-P3.md not present")
    hours = [
        int(h)
        for h in re.findall(
            r"^### CP3\.\d+ · .+? · (\d+)h · trip-wire",
            plan.read_text(encoding="utf-8"),
            re.M,
        )
    ]
    assert len(hours) == 12, f"expected 12 Phase 3 checkpoints, found {len(hours)}"
